...
def _fill_nan_nearest_2d(arr):
    ...
    if np.all(mask):
        return arr
    if not np.any(mask):
        raise ValueError("input array is all NaN")
    from scipy.ndimage import distance_transform_edt
    _, indices = distance_transform_edt(~mask, return_indices=True)
    return arr[tuple(indices)]


def _interp_linear_inside_1d(vec):
    """
    对 1D 向量做“仅内部线性插值”：
    - 只填充两端有效值之间的 NaN；
    - 两端外推区域保持 NaN。
    ...
    xv = x[valid]
    yv = out[valid]
    yi = np.interp(x, xv, yv, left=np.nan, right=np.nan)
    fill_mask = (~valid) & np.isfinite(yi)
    out[fill_mask] = yi[fill_mask]
    return out


def interpolate_sar_grid(arr, method="linear_nearest"):
    """
    对 SAR 网格做插值填洞。
    - nearest: 纯最近邻
    - linear_nearest: 行/列内部线性插值 + 最近邻补全剩余空洞
    """
    method = str(method).strip().lower()
    if method not in ("nearest", "linear_nearest"):
        raise ValueError(f"不支持的插值方法: {method}")

    x = np.asarray(arr, dtype=np.float32).copy()
    if x.ndim != 2:
        raise ValueError("interpolate_sar_grid expects 2D array")
    if np.all(np.isfinite(x)):
        return x
    if not np.any(np.isfinite(x)):
        raise ValueError("输入网格全为空，无法插值")

    if method == "nearest":
        return np.asarray(_fill_nan_nearest_2d(x), dtype=np.float32)

    # 1) 行方向内部线性插值
    y = x.astype(np.float64, copy=False)
    for i in range(y.shape[0]):
        if np.any(~np.isfinite(y[i])):
            y[i] = _interp_linear_inside_1d(y[i])

    # 2) 列方向内部线性插值
    for j in range(y.shape[1]):
        col = y[:, j]
        if np.any(~np.isfinite(col)):
            y[:, j] = _interp_linear_inside_1d(col)

    # 3) 剩余 NaN 用最近邻补齐
    y = _fill_nan_nearest_2d(y)
    return np.asarray(y, dtype=np.float32)


def rasterize_to_sar_grid_last_win(values, range_pixel, azimuth_pixel, sar_shape, dtype=np.float32):
    """
    将 DEM 顺序的一维值映射到 SAR 网格（last-win 规则）。
    """
    sar_az_size, sar_range_size = int(sar_shape[0]), int(sar_shape[1])
    val = np.asarray(values)
    rg = np.asarray(range_pixel)
    az = np.asarray(azimuth_pixel)
    if not (val.shape == rg.shape == az.shape):
        raise ValueError("values/range_pixel/azimuth_pixel 形状必须一致")

    out = np.full((sar_az_size, sar_range_size), np.nan, dtype=dtype)

    r = np.rint(rg).astype(np.int32, copy=False)
    a = np.rint(az).astype(np.int32, copy=False)
    ok = (
        np.isfinite(val) & np.isfinite(rg) & np.isfinite(az) &
        (r >= 0) & (r < sar_range_size) &
        (a >= 0) & (a < sar_az_size)
    )
    if not np.any(ok):
    ...
    r_ok = r[idx]
    a_ok = a[idx]
    v_ok = val[idx].astype(dtype, copy=False)

    # stable 排序 + 每个像素保留最后一次命中（last-win）
    key = a_ok.astype(np.int64) * int(sar_range_size) + r_ok.astype(np.int64)
    order = np.argsort(key, kind="mergesort")
    key_s = key[order]
    keep = np.ones(key_s.shape[0], dtype=bool)
    if key_s.shape[0] >= 2:
        keep[:-1] = key_s[1:] != key_s[:-1]

    a_sel = a_ok[order][keep]
    r_sel = r_ok[order][keep]
    v_sel = v_ok[order][keep]
    out[a_sel, r_sel] = v_sel
    return out, int(v_sel.size), int(idx.size)


def compute_sar_incidence_angle_grid(
    geo,
    dem_h,
    dem_gt,
    dem_shape,
    ...
    azimuth_time,
    sar_shape,
    block_points=500_000,
):
    """
    计算 SAR 网格下入射角（degree）：
    - 先按 DEM 点计算入射角
    - 再按 DEM->SAR 映射栅格化到 SAR 网格（last-win）
    """
    nrows, ncols = int(dem_shape[0]), int(dem_shape[1])
    total = int(nrows * ncols)

    h_flat = np.asarray(dem_h, dtype=np.float32, order="C").reshape(-1)
    rg = np.asarray(range_pixel).reshape(-1)
    az = np.asarray(azimuth_pixel).reshape(-1)
    azt = np.asarray(azimuth_time).reshape(-1)
    if not (h_flat.size == rg.size == az.size == azt.size == total):
        raise ValueError("dem_h/range_pixel/azimuth_pixel/azimuth_time 与 dem_shape 不一致")

    sar_az_size, sar_range_size = int(sar_shape[0]), int(sar_shape[1])
    r = np.rint(rg).astype(np.int32, copy=False)
    a = np.rint(az).astype(np.int32, copy=False)
    valid = (
        np.isfinite(h_flat) & np.isfinite(rg) & np.isfinite(az) & np.isfinite(azt) &
        (r >= 0) & (r < sar_range_size) &
        (a >= 0) & (a < sar_az_size)
    )
    ...
    if valid_idx.size == 0:
        out = np.full((sar_az_size, sar_range_size), np.nan, dtype=np.float32)
        return out, {"valid_dem_points": 0, "rasterized_pixels": 0, "hit_points": 0}

    block_points = max(10_000, int(block_points))
    gt0, gt1, gt2, gt3, gt4, gt5 = [float(x) for x in dem_gt]
    inc_valid = np.full(valid_idx.size, np.nan, dtype=np.float32)

    for i0 in range(0, valid_idx.size, block_points):
        i1 = min(valid_idx.size, i0 + block_points)
        idx = valid_idx[i0:i1]

        rr = idx // ncols
        cc = idx - rr * ncols
        c0 = cc.astype(np.float64) + 0.5
        r0 = rr.astype(np.float64) + 0.5
        lon = gt0 + gt1 * c0 + gt2 * r0
        lat = gt3 + gt4 * c0 + gt5 * r0
        hh = h_flat[idx].astype(np.float64, copy=False)

        xyz = llh_to_xyz(lat, lon, hh).astype(np.float64, copy=False)
        t = geo.t0 + azt[idx].astype(np.float64, copy=False)
        t = np.clip(t, geo.orbit.tmin, geo.orbit.tmax)
        sat = geo.orbit.position(t)

        # 入射角定义：LOS(地面->卫星) 与地表法向（近似取地心法向）夹角
        los = sat - xyz
        los_norm = np.linalg.norm(los, axis=1)
        nrm_norm = np.linalg.norm(xyz, axis=1)
        ok = (los_norm > 1e-10) & (nrm_norm > 1e-10)
        if np.any(ok):
            cosi = np.sum(los[ok] * xyz[ok], axis=1) / (los_norm[ok] * nrm_norm[ok])
            cosi = np.clip(cosi, -1.0, 1.0)
            inc_block = np.full(idx.size, np.nan, dtype=np.float32)
            inc_block[ok] = np.degrees(np.arccos(cosi)).astype(np.float32, copy=False)
            inc_valid[i0:i1] = inc_block

    inc_raw, wrote, hits = rasterize_to_sar_grid_last_win(
        inc_valid,
        r[valid_idx],
        a[valid_idx],
        sar_shape=(sar_az_size, sar_range_size),
    ...
    return inc_raw, {
        "valid_dem_points": int(valid_idx.size),
        "rasterized_pixels": int(wrote),
        "hit_points": int(hits),
    }
...
class Geo2Rdr:
    def __init__(
        ...
        self.orbit_time_is_epoch = bool(orbit_time_is_epoch)
    ...
    def sync_geometry_to_yaml(self, yaml_path=None):
        """
        将更新后的几何参数回写到 YAML，供后续流程直接读取。
        回写字段：
        - radar_parameters.near_range
        - 根级 near_range
        - 根级 t0
        - 根级 t1（若存在，按相同偏移平移，保持成像时长）
        - metadata.first_line_sensing_time（仅 orbit 时间体系为 epoch 时）
        - metadata.last_line_sensing_time（若存在，则按同样时间偏移同步平移）
        - sensor_start/sensor_end（metadata 与根级，若存在则同步平移）
        """
        target_yaml = str(yaml_path or self.yaml_file)
        if os.path.islink(target_yaml):
            # 关键约束：必须更新“当前 multilook YAML 路径”，而不是其链接目标（通常是原始 YAML）。
            # 因此先将软链接就地替换为独立文件快照，再回写更新字段。
            link_target = os.path.realpath(target_yaml)
            print(f"检测到 YAML 软链接，转换为独立文件后回写: {target_yaml}")
            with open(link_target, "r", encoding="utf-8") as f:
                raw_text = f.read()
            tmp_link = f"{target_yaml}.tmp"
            with open(tmp_link, "w", encoding="utf-8") as f:
                f.write(raw_text)
            os.replace(tmp_link, target_yaml)
        ...
        def _format_epoch_like_template(epoch, template):
            ...
            try:
                dt_tpl = datetime.datetime.fromisoformat(tpl)
            except Exception:
                dt_tpl = None

            if (dt_tpl is not None) and (dt_tpl.tzinfo is not None):
        ...
        if not isinstance(ycfg, dict):
            raise ValueError(f"YAML 顶层必须是映射(dict): {target_yaml}")

        radar_params = ycfg.get("radar_parameters", {})
        if not isinstance(radar_params, dict):
            radar_params = {}
        radar_params["near_range"] = float(self.near_range)
        ycfg["radar_parameters"] = radar_params
        ycfg["near_range"] = float(self.near_range)
        ycfg["t0"] = float(self.t0)

        meta = ycfg.get("metadata", {})
        if not isinstance(meta, dict):
            meta = {}

        dt_shift = float(self.t0) - float(self.original_t0)
        if "t1" in ycfg:
            try:
                ycfg["t1"] = float(ycfg["t1"]) + dt_shift
            except Exception:
                pass

        def _shift_optional(container, key):
            if key not in container:
                return
            try:
                t_old = _parse_time_iso8601(container.get(key))
                if t_old is not None:
                    container[key] = _format_epoch_like_template(float(t_old) + dt_shift, container.get(key))
            except Exception:
                pass

        if self.orbit_time_is_epoch:
            first_tpl = meta.get("first_line_sensing_time", self.t0_str)
            if ("first_line_sensing_time" in meta) or (first_tpl is not None):
                meta["first_line_sensing_time"] = _format_epoch_like_template(self.t0, first_tpl)
            _shift_optional(meta, "last_line_sensing_time")

            # 兼容部分产品使用 sensor_start/sensor_end 命名
            if "sensor_start" in meta:
                meta["sensor_start"] = _format_epoch_like_template(self.t0, meta.get("sensor_start"))
            _shift_optional(meta, "sensor_end")

            if "sensor_start" in ycfg:
                ycfg["sensor_start"] = _format_epoch_like_template(self.t0, ycfg.get("sensor_start"))
            _shift_optional(ycfg, "sensor_end")

            ycfg["metadata"] = meta

        tmp_yaml = f"{target_yaml}.tmp"
        with open(tmp_yaml, "w", encoding="utf-8") as f:
            yaml.safe_dump(ycfg, f, allow_unicode=True, sort_keys=False)
        os.replace(tmp_yaml, target_yaml)

        print(f"已同步更新 YAML: {target_yaml}")
        print(f"  near_range -> {self.near_range}")
        print(f"  t0 -> {self.t0}")
        if self.orbit_time_is_epoch:
            print(f"  metadata.first_line_sensing_time -> {ycfg['metadata'].get('first_line_sensing_time')}")
...
def main():
    ...
    parser.set_defaults(sync_yaml=True)
    parser.add_argument('--sync-yaml', dest='sync_yaml', action='store_true',
                        help='将更新后的 near_range/t0 同步回写到 --yaml（默认开启）')
    parser.add_argument('--no-sync-yaml', dest='sync_yaml', action='store_false',
                        help='不回写 --yaml，仅输出 geosar HDF')
    ...
    if getattr(args, "sync_yaml", True):
        print("同步更新 YAML 几何参数...")
        geo2rdr.sync_geometry_to_yaml(args.yaml)
    ...
    # SAR 坐标 DEM 插值填洞（当前映射表会导致大量空洞）
    sar_interp_method = "linear_nearest"
    print(f"对 SAR 坐标 DEM 做插值填洞（方法: {sar_interp_method}）...")
    sar_dem_raw = np.asarray(sar_dem_updated, dtype=np.float32, order='C')
    raw_nan = int(np.sum(~np.isfinite(sar_dem_raw)))
    try:
        sar_dem_filled = interpolate_sar_grid(sar_dem_raw, method=sar_interp_method)
    except Exception as e:
        print(f"警告：SAR DEM 插值失败，回退到原始网格。原因: {e}")
        sar_dem_filled = sar_dem_raw
    filled_nan = int(np.sum(~np.isfinite(sar_dem_filled)))
    print(f"SAR DEM 空洞统计: 原始 NaN={raw_nan}, 插值后 NaN={filled_nan}")

    # 计算 SAR 坐标系下每个像素的入射角（degree）
    print("计算 SAR 网格入射角...")
    incidence_interp_method = "linear_nearest"
    try:
        incidence_raw, inc_stats = compute_sar_incidence_angle_grid(
            geo=geo2rdr,
            dem_h=dem_h,
            dem_gt=dem_gt,
            dem_shape=dem_shape,
            ...
            azimuth_time=azimuth_time_updated,
            sar_shape=(sar_az_size, sar_range_size),
            block_points=500_000,
        )
        incidence_raw_nan = int(np.sum(~np.isfinite(incidence_raw)))
        incidence_filled = interpolate_sar_grid(incidence_raw, method=incidence_interp_method)
        incidence_filled_nan = int(np.sum(~np.isfinite(incidence_filled)))
        print(
            f"入射角统计: 有效DEM点={inc_stats['valid_dem_points']}, "
            f"命中点={inc_stats['hit_points']}, 栅格化像素={inc_stats['rasterized_pixels']}, "
            f"原始NaN={incidence_raw_nan}, 插值后NaN={incidence_filled_nan}"
        )
    except Exception as e:
        print(f"警告：入射角计算失败，将输出全 NaN。原因: {e}")
        incidence_raw = np.full((sar_az_size, sar_range_size), np.nan, dtype=np.float32)
        incidence_filled = incidence_raw.copy()
        inc_stats = {"valid_dem_points": 0, "rasterized_pixels": 0, "hit_points": 0}
    ...
    with h5py.File(args.output, 'w') as f:
        ...
        # 添加模拟SAR DEM（raw + 填洞后）
        f.create_dataset('sar_dem_raw', data=sar_dem_raw)
        sar_dem_ds = f.create_dataset('sar_dem', data=sar_dem_filled)
        ...
        try:
            ...
            sar_dem_ds.attrs['interpolation_method'] = np.bytes_(sar_interp_method)
            sar_dem_ds.attrs['source'] = np.bytes_('sar_dem_raw')
        except Exception:
            pass
        # 添加 SAR 网格入射角（raw + 填洞后）
        inc_raw_ds = f.create_dataset('incidence_angle_raw', data=incidence_raw)
        inc_ds = f.create_dataset('incidence_angle', data=incidence_filled)
        try:
            inc_raw_ds.attrs['unit'] = np.bytes_('degree')
            inc_raw_ds.attrs['source'] = np.bytes_('dem_to_sar_mapping_raw')
            inc_raw_ds.attrs['valid_dem_points'] = int(inc_stats.get('valid_dem_points', 0))
            inc_raw_ds.attrs['hit_points'] = int(inc_stats.get('hit_points', 0))
            inc_raw_ds.attrs['rasterized_pixels'] = int(inc_stats.get('rasterized_pixels', 0))
            inc_ds.attrs['unit'] = np.bytes_('degree')
            inc_ds.attrs['interpolation_method'] = np.bytes_(incidence_interp_method)
            inc_ds.attrs['source'] = np.bytes_('incidence_angle_raw')
        except Exception:
            pass
        # 添加 SAR 图像尺寸信息
