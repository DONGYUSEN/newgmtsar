...
def download_url(url: str, dst: str, *, timeout_s: float = 60.0, retries: int = 3) -> None:
...
def download_srtm_tile(lat: int, lon: int, cache_dir: str, *, keep_zip: bool = False) -> str:
    """下载单个SRTM瓦片
    ...
    print(f"下载 SRTMGL1: {tile_id}  {url}")

    if not os.path.exists(zip_file):
        download_url(url, zip_file, retries=3)
...
def download_dem(
    ...
    *,
    ...
    # 如果未指定 hgt_dir，则使用临时目录，拼接完成后删除（避免污染工作目录）。
    tmp_dir = None
    if hgt_dir is None:
    ...
    for lat in range(min_lat_int, max_lat_int):
        for lon in range(min_lon_int, max_lon_int):
            tile_file = download_srtm_tile(lat, lon, tile_dir, keep_zip=keep_zip)
    ...
    if len(tiles) == 1:
        # 单个瓦片，直接复制
        shutil.copy(tiles[0], dem_tif_file)
    else:
        # 多个瓦片，使用gdal.BuildVRT和gdal.Translate合并
        vrt_file = os.path.join(output_dir, "dem.vrt")
        gdal.BuildVRT(vrt_file, tiles)
        gdal.Translate(dem_tif_file, vrt_file)
        try:
            os.remove(vrt_file)
        except OSError:
            pass
    ...
    *,
    ...
    if src_dem:
        ...
        if bbox_expanded is None:
            raise ValueError("未提供 --src-dem，且 YAML 无角点范围；无法确定下载范围。请提供 --bbox 或 --src-dem。")
        print("2. 下载并拼接 SRTMGL1")
...
def main():
    ...
    parser.add_argument(
        ...
        "--src-dem",
        default=None,
        help="本地 DEM 文件路径（GeoTIFF/HGT/VRT 等 GDAL 可读格式）。提供后将跳过在线下载；若 YAML 无角点则默认全量处理。",
    )
    parser.add_argument(
        "-b",
        "--bbox",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="保留下载的 SRTM 瓦片 zip 作为缓存（不开启则下载后删除 zip）。",
    )
    parser.add_argument(
        "-t",
        "--hgt-dir",
