...
static void validate_xcorr_config(const struct st_xcorr *xc) {
    ...
    if (xc->m_nx < nx_win || xc->m_ny < ny_win)
        die_arg("Master image is smaller than the requested correlation window");
    if (xc->s_nx < nx_win || xc->s_ny < ny_win)
        die_arg("Slave image is smaller than the requested correlation window");
    ...
    if (x_lower > x_upper || y_lower > y_upper) {
        fprintf(stderr,
                "Warning: No strict feasible window centers for current search/shift/stretch settings.\n"
                "         Fallback to overlap mode to preserve requested --nx/--ny; "
                "slave windows will be clamped near boundaries.\n");
    }
}
